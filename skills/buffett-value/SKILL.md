---
name: buffett-value
description: Screen for Buffett-style value stocks using business quality, consistency, and valuation filters with candlestick entry timing and profit/fundamentals-driven exits
---

This skill implements a Warren Buffett-inspired value investing strategy that screens for high-quality businesses trading at a significant margin of safety, with specific entry and exit rules.
...
Generates analysis documents in the `reports/` directory:
- `buffett_value_analysis_YYYY-MM-DD.md` - Main screening results
- `buffett_value_signals_YYYY-MM-DD.json` - Machine-readable signals
- Includes fundamental scores, valuation metrics, candlestick patterns, and conviction levels

## Dependencies

- **Required**: none. Fundamentals, price, and volume data all come from yfinance (no API key)

## Example Usage

Trigger this skill by asking Claude to:
- "Run Buffett value screener"
- "Find undervalued quality companies with margin of safety"
- "Screen for Buffett-style stocks using fundamental and technical filters"

The skill will output ranked candidates with:
- Fundamental quality score (0-100)
- Valuation discount percentage
- Detected candlestick entry signals
- Conviction level for position sizing
- Fair value target price for exit planning

## References

Check `references/` directory for:
- Value investing principles and frameworks
- Candlestick pattern definitions
- Intrinsic value calculation methodologies