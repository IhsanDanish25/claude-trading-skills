...
STRATEGY_CONFIGS = {
    ...
    "timeseries": {
        "label": "Time-series Confirming Filter (standalone validation)",
        "stop_pct_attr": "TIMESERIES_STOP_PCT",
        "hold_days_attr": "TIMESERIES_HOLD_DAYS",
        "min_price_attr": "TIMESERIES_MIN_PRICE",
...
def _run_satellite_strategy(
    key: str, store, universe, start_equity: float, slippage_bps: float, out_dir: str
) -> dict:
    ...
    log.info(
        "%s: %d point-in-time signals across %d symbols",
        label,
        len(signals),
    ...
    report = {
        ...
        "config": {
            ...
            "start_equity": start_equity,
            "max_position_pct": earnings_engine.BACKTEST_MAX_POSITION_PCT,
            "max_open_positions": earnings_engine.BACKTEST_MAX_OPEN_POSITIONS,
        },
        "performance": strat,
        "spy_buy_hold": spy,
        "trade_stats": tstats,
...
def main() -> int:
    ...
    log.info(
        "Universe (%s): %d cached symbols (excl. index/sector ETFs)", args.universe, len(universe)
    )
    ...
    for key in ("breakout", "meanrev", "earnmom", "timeseries"):
        try:
            ...
            results[key] = {
                "label": STRATEGY_CONFIGS[key]["label"],
                "status": "error",
                "reason": str(e),
            }
...
def _print_summary_table(results: dict[str, dict]) -> None:
    ...
    for key in ("breakout", "meanrev", "earnmom", "insider", "squeeze"):
        ...
        if status == "ok":
            ...
            winr = (
                f"{tstats.get('win_rate_pct'):.1f}%"
                if tstats.get("win_rate_pct") is not None
                else "n/a"
            )
